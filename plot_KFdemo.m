...
for kx = 1:Nx
    ...
    plot(tk, xk(kx,:), 'k-', tk, xk_hat(kx,:), 'r--', 'Linewidth',plt.LineWidth)
...
for kx = 1:Nx
    ...
    xbound = [tk tk(end:-1:1)];
    ybound = [-3.*y 3.*y(end:-1:1)];
    bound           = fill(xbound,ybound, 'red');
    bound.FaceColor = [1 0.8 0.8];
    bound.EdgeColor = 'none';
    hold on; box on;
    plot(tk, avg_error(kx,:), 'r-', 'LineWidth', plt.LineWidth, 'MarkerSize', plt.MarkerSize)
    set(gca, 'fontsize', plt.FontSize, 'FontName', plt.FontName)
    xlabel('Time (s)')
    ylabel('Position Error')
    legend({'$ \pm 3 \sigma $ Estimated bounds', 'RMSE'}, 'interpreter', 'latex', Location='best')
    axis('tight')
    switch kx
        case 1
            ylabel('Position (m)');
...
bound.EdgeColor = 'none';
hold on; box on;
plot(tk, avg_NIS,'r-','Linewidth',plt.LineWidth)
set(gca, 'fontsize', plt.FontSize, 'FontName', plt.FontName)
...
bound.EdgeColor = 'none';
hold on; box on;
plot(tk, avg_NEES,'r-','Linewidth',plt.LineWidth)
set(gca, 'fontsize', plt.FontSize, 'FontName', plt.FontName)
